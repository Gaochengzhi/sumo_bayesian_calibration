...
config_path = (
    ...
    + "/../../../Environment/environment/merge/highway1.sumocfg"
...
class Traffic_Env(gym.Env):
    ...
    def step(self, action_a):
        traci.vehicle.setSpeed(
            self.AutoCarID,
            max(traci.vehicle.getSpeed(self.AutoCarID) + action_a, 0.001),
        )
        traci.simulationStep()

        # Get the new vehicle parameters
        new_vehicle_params = traci.vehicle.getIDList()
        reward_cost, collision_value, reward, cost = self.get_reward_a(
            new_vehicle_params
        )
    ...
    def reset(self):
        ...
        traci.load(["-c", config_path, "--lateral-resolution", "0.4"])
...
def test_traffic_env_with_gui():
    env = Traffic_Env()
    env.start(gui=True)  # Start with GUI
    ...
    while not done and step < max_steps:
        action = np.random.uniform(-1, 1)  # Random action between -1 and 1
        reward, next_state, done, _, _, _ = env.step(action)
